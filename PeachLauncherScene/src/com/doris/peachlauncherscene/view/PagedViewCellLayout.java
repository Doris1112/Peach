...
public class PagedViewCellLayout extends ViewGroup implements Page {
	...
	public PagedViewCellLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mOriginalCellWidth = mCellWidth = resources
				.getDimensionPixelSize(R.dimen.apps_customize_cell_width);
		mOriginalCellHeight = mCellHeight = resources
				.getDimensionPixelSize(R.dimen.apps_customize_cell_height);
		// mCellCountX = LauncherModel.getCellCountX();
		// mCellCountY = LauncherModel.getCellCountY();
		mOriginalWidthGap = mOriginalHeightGap = mWidthGap = mHeightGap = -1;
		mMaxGap = resources
				.getDimensionPixelSize(R.dimen.apps_customize_max_gap);
	...
	public boolean addViewToCellLayout(View child, int index, int childId,
		...
		if (lp.cellX >= 0 && lp.cellX <= (mCellCountX - 1) && lp.cellY >= 0
				&& (lp.cellY <= mCellCountY - 1)) {
	...
	int getContentWidth() {
		return getWidthBeforeFirstLayout() + getPaddingLeft()
				+ getPaddingRight();
	}

	int getContentHeight() {
		if (mCellCountY > 0) {
			return mCellCountY * mCellHeight + (mCellCountY - 1)
					* Math.max(0, mHeightGap);
		}
		return 0;
	}
	...
	int getWidthBeforeFirstLayout() {
		if (mCellCountX > 0) {
			return mCellCountX * mCellWidth + (mCellCountX - 1)
					* Math.max(0, mWidthGap);
		}
		return 0;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (count > 0) {
			...
			int numRows = (int) Math.ceil((float) getPageChildCount()
					/ getCellCountX());
	...
	public int estimateCellHSpan(int width) {
		...
		int n = Math
				.max(1, (availWidth + mWidthGap) / (mCellWidth + mWidthGap));
	...
	public int estimateCellVSpan(int height) {
		...
		// juggle to solve for N
		int n = Math.max(1, (availHeight + mHeightGap)
	...
	public int[] estimateCellPosition(int x, int y) {
		return new int[] {
				getPaddingLeft() + (x * mCellWidth) + (x * mWidthGap)
						+ (mCellWidth / 2),
				getPaddingTop() + (y * mCellHeight) + (y * mHeightGap)
						+ (mCellHeight / 2) };
	}

	public void calculateCellCount(int width, int height, int maxCellCountX,
			int maxCellCountY) {
	...
	public static class LayoutParams extends ViewGroup.MarginLayoutParams {
		...
		public String toString() {
			return "(" + this.cellX + ", " + this.cellY + ", " + this.cellHSpan
					+ ", " + this.cellVSpan + ")";
		}
	}
}
