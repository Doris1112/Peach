...
import com.doris.peachlauncherscene.R;
...
//	private static final boolean DEBUG = false;
	...
	// shifts
	private static final int MIN_LENGTH_FOR_FLING = 15;// 25;

	private static final int PAGE_SNAP_ANIMATION_DURATION = 250;// 550;
	protected static final float NANOTIME_DIV = 1000000000.0f;

	private static final float OVERSCROLL_ACCELERATE_FACTOR = 3;// 2;
	private static final float OVERSCROLL_DAMP_FACTOR = 0.14f;
	private static final int MINIMUM_SNAP_VELOCITY = 2500;// 2200;
	private static final int MIN_FLING_VELOCITY = 300;// 250;
	...
	// page
	protected int mSnapVelocity = 200;// 500;
	...
	protected static final int sScrollIndicatorFadeInDuration = 100;// 150;
	protected static final int sScrollIndicatorFadeOutDuration = 500;// 650;
	protected static final int sScrollIndicatorFlashDuration = 500;// 650;

	// If set, will defer loading associated pages until the scrolling settles
	private boolean mDeferLoadAssociatedPagesUntilScrollCompletes;

	private boolean mIdle_screen_cycle_turn_page = false;
	...
	public PagedView(Context context, AttributeSet attrs, int defStyle) {
		...
		mScrollIndicatorPaddingLeft = a.getDimensionPixelSize(
				R.styleable.PagedView_scrollIndicatorPaddingLeft, 0);
		mScrollIndicatorPaddingRight = a.getDimensionPixelSize(
				R.styleable.PagedView_scrollIndicatorPaddingRight, 0);
		a.recycle();
	...
	public void scrollTo(int x, int y) {
		...
		if (x < 0) {
			...
			// start:wutianjun, 2012-12-13, 待机屏循环翻页
			else if (mIdle_screen_cycle_turn_page) {
				super.scrollTo(x, y);
			}
			// end:wutianjun, 2012-12-13, 待机屏循环翻页
...
//			 "PagedView.onLayout()"
		...
		if (childCount > 0) {
//			if (DEBUG)
//				"getRelativeChildOffset(): " + getMeasuredWidth() + ", " + getChildWidth(0)
...
//				if (DEBUG)
//					"\tlayout-child" + i + ": " + childLeft + ", " + childTop;
	...
	// wutianjun, 2012-12-21, 增加一个函数，供主菜单循环使用
	public int getNextPage(int currentPage, boolean incPage) {
		return currentPage;
	}
...
//							"onTouchEvent().Scrolling: " + deltaX;
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				// start:wutianjun, 2012-12-20，支持循环
				boolean is_proc = true;
...
//		if (DEBUG)
//			"snapToPage.getChildOffset(): " + getChildOffset(whichPage);
//		if (DEBUG)
//			"snapToPageWithVelocity.getRelativeChildOffset(): "
...
//			"snapToPage.getChildOffset(): " + getChildOffset(whichPage);
//		if (DEBUG)
//			"snapToPage.getRelativeChildOffset(): "
//					+ getMeasuredWidth() + ", " + getChildWidth(whichPage);
	...
	protected ImageView getScrollingIndicator() {
		...
		if (mHasScrollIndicator && mScrollIndicator == null) {
			...
			mScrollIndicator = (ImageView) (parent
					.findViewById(R.id.paged_view_indicator));
