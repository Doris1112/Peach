package com.doris.peach.activity.paperfolding.util;

import com.doris.peach.activity.paperfolding.ModelView;
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (e.getPointerCount() == 1) {
			...
			if (e.getAction() == MotionEvent.ACTION_UP) {
				...
				// Simple tap continue, if we we were not already running and
				// paused by touch down
			...
			// One first touch, switch to pause
				...
				if (mMainPane.commands.state == Commands.State.run || mMainPane.commands.state == Commands.State.anim) {
					wasRunning = true;
